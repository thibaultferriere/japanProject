...
$sql = 'select calculated_at as time, spot_id as spot, congestion as congestion from campus_forecasts_scaled where calculated_at IN(
600 + (select MAX(calculated_at) from campus_nowcasts_scaled),
1200 + (select MAX(calculated_at) from campus_nowcasts_scaled),
1800 + (select MAX(calculated_at) from campus_nowcasts_scaled),
2400 + (select MAX(calculated_at) from campus_nowcasts_scaled),
3000 + (select MAX(calculated_at) from campus_nowcasts_scaled),
3600 + (select MAX(calculated_at) from campus_nowcasts_scaled)
) order by spot_id';
